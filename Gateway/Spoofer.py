...
#             '1.1.1.1': ['10.10.16.101', 55]}
    ...
    def callback(self, pkt):
        ...
        if self.isWhitelistedPacket(sc_pkt):
            ...
            return
	print("white")
        if self.isSpoofPacket(sc_pkt):
            sc_pkt = self.checksumStrip(sc_pkt)
            spoof = True
	print("spoof")
        if TCP in sc_pkt and self.type == 1:
         ...
         #Lookup by port
            elif sc_pkt[TCP].sport in self.sourcePortMap:
                self.spoofIPPointer = self.sourcePortMap[sc_pkt[TCP].sport]
            elif sc_pkt[TCP].dport in self.sourcePortMap:
                print(self.sourcePortMap[sc_pkt[TCP].dport])
                print(self.spoofIPs[self.sourcePortMap[sc_pkt[TCP].dport]])
                if sc_pkt[IP].dst == self.spoofIPs[self.sourcePortMap[sc_pkt[TCP].dport]][0]:
		    self.spoofIPPointer = self.sourcePortMap[sc_pkt[TCP].dport]
	    else:
                pkt.drop()
                return
        else:
    ...
    def isWhitelistedPacket(self, packet):
        if UDP in packet:
	    #DNS Query
            if packet[IP].dst == "10.10.255.254":
                return True
            #DNS Response
            if packet[IP].src == "10.10.255.254":
                return True
